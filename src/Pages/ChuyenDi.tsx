...
import { getRoomByUserApi } from '../Reducer/userReducer';

type Props = {}

const ChuyenDi = (props: Props) => {
    const { userProfile } = useSelector((state: RootState) => state.userReducer);
    const dispatch: dispatchType = useDispatch();
    const { userRoom } = useSelector((state: RootState) => state.userReducer);
    const { id } = useParams();
    const getRoomByUser = async () => {
        const action = await getRoomByUserApi(userProfile?.id);
        dispatch(action);
    };
                        ...
                        return (
                            ...
                            <>
                                <div className="border mb-4 rounded-lg hover:shadow-xl duration-200 flex cursor-pointer">
                                    <div key={item.id}>
                                        <img src="https://airbnb.cybersoft.edu.vn/public/images/room/1658134435797_put-together-a-perfect-guest-room-1976987-hero-223e3e8f697e4b13b62ad4fe898d492d.jpg" alt="https://airbnb.cybersoft.edu.vn/public/images/room/1658134435797_put-together-a-perfect-guest-room-1976987-hero-223e3e8f697e4b13b62ad4fe898d492d.jpg" className="h-full rounded-l-lg" /></div><div className="px-4 py-2  w-full"><div className="flex justify-between items-center md:border-b pb-2">
                                            <div>
                                                <h3 className="font-semibold md:text-xl uppercase">Khách Sạn Hồng Hào</h3>
                                                <p className="text-gray-600 text-xs md:text-sm md:ml-2">Mã phòng là : {item.maPhong}</p>
                                                <span className="text-gray-600 text-xs md:text-sm md:ml-2">
                                                    <span>{item.ngayDen} --&gt;   </span> <span>{item.ngayDi}</span></span></div>
                                            <div><span className="font-semibold text-sm md:text-xl">0
                                                ...
                                                <p className="font-medium text-gray-600 text-xs md:text-sm">Số lượng khách : {item.soLuongKhach}</p>
                                            </span>
                                            </div>
                                        </div>

...
export const updateRoomById = (data: any) => {
  return async (dispatch: dispatchType) => {
    const res = await http.put(`/api/phong-thue/${data.id}`, data);
    if (res) {
      Swal.fire({
        icon: "success",
...
export const deletePlaceById = (id: number) => {
  return async (dispatch: dispatchType) => {
    const res = await http.delete(`/api/vi-tri/${id}`);
    if (res) {
      Swal.fire({
        icon: "success",
        text: "Xóa vị trí thành côg",
      });

...
import { getStoreJson, roomsJSON } from "../util/config";

type Props = {};

const QuanLyPhong = (props: Props) => {
  const rooms = getStoreJson(roomsJSON);
  ...
  const openModalById = (id: number, open: boolean, isDisabled: boolean, title: string) => {
    const index = rooms?.findIndex((room: any) => room.id == id)
    return <Modal
      title= {title}
      centered
      open={open}
          ...
          <div className="mb-4 w-full">
            ...
            <div className="mb-3">
              ...
              <input
                ...
                id="tenPhong"
                type="text"
                defaultValue={rooms[index]?.tenPhong}
                disabled = {isDisabled}
              />
            </div>
            <div className="mb-3">
